...
class CustomerOrder : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnsave.setOnClickListener {
            ...
            val product = CustomerOrderEntity(Book_Name = bookname, Book_Number = booknumber)

            CoroutineScope(Dispatchers.IO).launch {
                try {
                    val customerorderRepository = CustomerOrderRepo()
                    val response = customerorderRepository.addProduct(product)
...
//                        }
                        ...
                        startActivity(Intent(this@CustomerOrder, ViewCustomerOrder::class.java));
                    }
                } catch (ex: Exception) {
                    withContext(Dispatchers.Main) {
                        Toast.makeText(
        ...
        if (imageUrl != null) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    if (response.success == true) {
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@CustomerOrder, "Uploaded", Toast.LENGTH_SHORT)
                                .show()
                        }
                    }
                } catch (ex: Exception) {
    ...
    private fun loadPopUpMenu() {
        val popupMenu = PopupMenu(this@CustomerOrder, imgProfile)
